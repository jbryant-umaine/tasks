...
export function toCSV(questions: Question[]): string {
    let output = "id,name,options,points,published\n";

    questions.forEach((q) => {
        output += `${q.id},${q.name},${q.options.length},${q.points},${q.published}\n`;
    });
...
export function changeQuestionTypeById(
    ...
    return questions.map((q) => {
        if (q.id === targetId) {
            let copy = { ...q, type: newQuestionType };
...
export function editOption(
    ...
    return questions.map((q) => {
        if (q.id === targetId) {
            let copy = { ...q, options: [...q.options] };

            if (targetOptionIndex === -1) copy.options.push(newOption);
            else copy.options[targetOptionIndex] = newOption;
...
export function duplicateQuestionInArray(
    ...
    let i = questions.findIndex((q) => q.id === targetId);

    return [
        ...questions.slice(0, i + 1),
        duplicateQuestion(newId, questions[i]),
        ...questions.slice(i + 1),
    ];
}
